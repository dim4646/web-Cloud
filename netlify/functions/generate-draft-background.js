...
Respond with ONLY the raw HTML, starting with <!DOCTYPE html> — no markdown code fences, no explanation before or after.`;
    ...
    // A full single-page site (hero + about + services + work + contact +
    // footer, each with real CSS) can run past the token budget, in which
    // case Anthropic cuts the response off mid-tag rather than erroring -
    // treat that the same as a failure instead of shipping half a page.
